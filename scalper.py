...
def check_trading_conditions(df):
    ...
    df['Long_entry'] = (df['close'] > df['resistance_1']) & (df['open'] < df['resistance_1']) & (df['low'] < df['resistance_1']) & (df['close'] > df['ema']) & (df['ema'] > df['ema_200']) & (df['ema_slope'] > 15)

    df['Sell_entry'] = (df['close'] < df['support_1']) & (df['open'] > df['support_1']) & (df['high'] > df['support_1']) & (df['close'] < df['ema']) & (df['ema'] < df['ema_200']) & (df['ema_slope'] < -15)
    ...
    resistance_levels = [x for x in [
        df['resistance_3'].iloc[-1], df['resistance_2'].iloc[-1]] if not np.isnan(x)]
    support_levels = [x for x in [df['support_3'].iloc[-1],
                                  df['support_2'].iloc[-1]] if not np.isnan(x)]
        ...
        try:
            ...
            try:
                ...
                if positions_response and 'result' in positions_response and 'list' in positions_response['result']:
                    ...
                    if positions and float(positions[0].get('size', 0)) > 0:
                        ...
                        print(
                            f"Side: {pos.get('side')}, Size: {pos.get('size')}, Entry: {pos.get('avgPrice')}")
                    else:
            ...
            try:
                ohlcv = bybit.fetch_ohlcv(
                    symbol=SYMBOL, timeframe=TIMEFRAME, limit=400)
                df = pd.DataFrame(
